...
#define STACKSIZE 0x4000 // match this to the value in loader.s
...
typedef struct {
    ...
    void  testisr_c();
    void  isr_entry_kbd();
    void  isr_entry_timer();
    void  isr_keyboard();
    void  isr_timer();

    void  panic();
    void  kernel_working_thread();
    ...
    process_table_entry_t *process_table;
    int32 process_num;
    int32 current_process;
    ...
    */
    ...
    void isr_timer() {
        kputs("timer interrupt"); NL;
        test_process_1();
        void (*next_process_entry)() = (void(*)()) process_table[current_process].instruction_pointer;
        current_process++;
        if (current_process == process_num) current_process = 0;
        PRINT(current_process);
        PRINT((int32)&next_process_entry);
        EOI;
        next_process_entry();
    }
    ...
    void build_process_table() {
        extern int32 stack; // continue to use kernel stack reserved in loader
                            // TODO reconsider this later
        process_table = kmalloc(1);
        //stack_kddernel_thread = kmalloc(1);
        //void *stack_process_thread = kmalloc(1);
        process_table[0].instruction_pointer = (int32) &test_process_1;    
        process_table[0].stack_pointer = stack + STACKSIZE - 4;
        process_table[1].instruction_pointer = (int32) &test_process_2;
        process_table[1].stack_pointer = stack + STACKSIZE - 4;
        process_num = 2;
        //if (module_num) {
        //    process_num = 1;
        //    process_table[1].instruction_pointer = 0x80000000;    
        //    process_table[1].stack_pointer = (int32) stack_process_thread;
        //}
        current_process = 0;
    }

    void kernel_working_thread() {
        char *testvideo = (char *) 0xf0000000;
    ...
    void kmain(void) {
        ...
        make_idt_entry(0x20, (void *)isr_entry_timer);
        ...
        outb(0x21, 0xfc);
        outb(0xa1, 0xff);
        // Enable interrupts only later, when the process table is set up
        //asm volatile("sti");
        //HALT;
    ...
    // Kernel initialization thread halts here. So it goes.
    // The scheduler will pick up the first thread to run at the first timer
    // interrupt. The CPU deserves that much rest.
    PRINT_EIP;
    asm volatile("sti");
    asm volatile("hlt");
    HALT;
